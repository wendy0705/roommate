...
- **房型及設施**：用戶可選擇適合的房型及提供的設施選項。
  - 系統透過 Google Maps API，讓使用者填寫房屋、可出租的房型及現有室友的資訊。

在這個階段，系統會根據這些租屋條件進行篩選，找出房屋需求有交集的潛在室友。
...
使用者進入第二階段，填寫詳細的興趣和生活習慣調查。根據這些數據，系統會計算使用者之間的匹配度，並依據匹配度高低進行排序，以協助使用者找到更符合生活習慣與興趣的合適室友。
